...
SYSTEM_BASE = (
    ...
    "\n⚠️ 关键要求：每一轮的最后必须用中文给出**最终回答**（包含关键结果数值/结论），"
    "不允许只调用工具就结束而不输出文字。工具拿到结果后，请把核心数值/明文/结论清晰展示出来；"
    "若任务已无需更多工具，直接输出文字回答。"
    "\n当用户要求做图表（柱状图/折线图/可视化）时，在回答中输出一个 ```chart "
    "代码块，内容为 JSON，格式："
    '{"type":"bar"或"line","title":"标题","labels":["A","B",...],'
    '"series":[{"name":"系列名","data":[数值,...]}]}。'
    "labels 与每个 series.data 等长；单组数据也用此结构。除该代码块外可附一句简短说明，"
    "不要把数据数组再用文字罗列一遍。若数据来自密态计算，用解密后的明文数值填入。"
)
